#!/usr/bin/env python3
"""
G4.4 Stage 0c (= 0b + a second tile-tuning pass, cfg 19-25) -- the MECHANISM test, run after Stage 0 (job 96) returned
...
TWIN = {0: 15, 1: 16, 10: 17, 11: 18, 19: 23, 22: 24}
...
def main():
    ...
    for name, M, K, N, judged in CASES:
        ...
        # Achievable-HBM reference: a pure fp16 copy of the OUTPUT tensor,
        # same harness. 2*M*N*2 bytes of traffic. This bounds how much of the
        # kernel time is unavoidable memory movement rather than mma issue.
        ...
        print(f"    -> compulsory traffic at that measured rate: "
              ...
              f"({flop/(bytes_min/bw/1e9)/1e12:.1f} TFLOPS hard ceiling)")
        ...
        bt2 = graph_time(lambda: ext.mma_gemm(bc, inp, w, b, out),
                         iters, replays)
