...
## CAUSAL PATH LEDGER (merged from `docs/CAUSAL_LEDGER.md`)

This section closes the gap noted in `DOCUMENTATION.md`'s "note on
currency": this narrative's numbered steps above stop at step 40 and never
covered the causal-path rewrite, which happened afterward, separately, and
is real/shipped (verifiable in `git log`, `archive/causal*.json`). Merged
in whole from the standalone `docs/CAUSAL_LEDGER.md` (that file is now a
stub pointing here — this section is the single source of truth for
causal-path work). Kept as its own table/format rather than renumbered
into the step-N prose above, since it was written and validated as an
independent, causal-specific track against the same accuracy budget
(`atol=0.002, rtol=0.02`, disjunctive) `benchmark.py --atol`/`--rtol`
already default to. Non-causal code and archive cells are explicitly out
of scope for this section and untouched by any of the work below — see
`CLAUDE.md`'s OFFICIAL CAUSAL EVALUATION MATRIX section for that boundary.
