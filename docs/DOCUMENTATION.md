...
| **G8.2 — FlashAttention-3** (Hopper `wgmma` + TMA + async warp specialisation, optional FP8) | Closed on hardware, cannot run at all | The card is **sm_89** (Ada); FA3 requires **sm_90a**. No `flash_attn*` package is installed and PyTorch 2.13 dispatches its vendored **FA2** kernel (`pytorch_flash::flash_fwd_splitkv_kernel`). Same wall as ThunderKittens. Measured the ceiling anyway, because it bounds *any* future attention kernel: attention is 7.6–38.2% of the shipped forward on rows 1–13, so even FA3's own reported 1.5–2.0× would cap the whole-model gain at **2.5–19.1%** there. Row 14 is the one attention-dominated shape (90.2%) and already runs at 91% of the roofline after G7.4 | step 59, `results/logs/g8_1_fa3_evaluation_run232.log` |
