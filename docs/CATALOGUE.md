...
| G8.2 | **FP16-accumulate + FP32 carry** (run the tensor core at 330 TF, rebuild precision outside it) | CLOSED | Accuracy IS reachable at the official K=128 with the finest carry (5 of 12 rows under the 0.00180 ship ceiling, job 235) -- the step-41 closure at K=512 was misleading here. But the tier is worth ~0% at these shapes: they achieve 0.6-27% of even the 165.2 TF lower tier, and accF32 vs accF16 differ by ~1% (job 236) where step 37 measured x1.43-1.53 at K=512/N=1536. Phase 1 BUILT anyway (CUTLASS + split-K, cfg24-27): at K=1024 the tier IS real -- 239.4 TF vs cuBLAS's 154.3, x1.55 -- so the K=128 generalisation was too broad. But accuracy closes it: the fastest config is 7.39e-03 and adding carry trades speed for accuracy faster than it gains, hitting 1.03x SLOWER at split-K 4 while still ~2x over the 0.00180 ceiling. Note CUTLASS's serial split-K rounds partials through the fp16 output, so it is a weaker rebuild than the hand kernel's FP32 register carry. |
