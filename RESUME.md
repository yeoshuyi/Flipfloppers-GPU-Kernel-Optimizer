...
| phase | status |
|---|---|
| P1 `benchmark.py`: `_CHUNK_*` consts + `_would_oom_causal` + `forward()` gate + `_chunked_forward_causal` | IN PROGRESS |
| P2 `experiments/g7_0_chunked_oversize.py` + `infra/slurm/g7_0_chunked_oversize.sbatch` | not started |
| P3 `run_eval.sh` `RUN_ROW14=1` → run the probe | not started |
| P4 sbatch the probe → real numbers in `results/logs/g7_0_chunked_oversize_run<J>.log` | not started |
